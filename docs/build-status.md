...
- `src/experience/lighting/volumetricLighting.js` — framework-agnostic controller (`createVolumetricLighting()`) exposing `init()` / `update(time)` / `dispose()` and an exported `lightingParams` data structure (color, intensity, position, target, angle, penumbra, decay, distance, shadow, ambient, fog, volumetric, dust) for Phase 1C to bind to later. Builds:
  ...
  - A desaturated-grey `AmbientLight`, **raised again per follow-up review feedback: color `#9a9aa2`→`#a6a6b0`, intensity 0.85→1.1→1.7** — the back wall, side walls, and column silhouettes are now clearly readable in the shadowed areas rather than reading as pure black.
...
- `src/experience/CinematicExperience.jsx` — added the `shadows` prop to the R3F `Canvas` to enable the renderer's shadow map; added `<fogExp2 attach="fog" args={[lightingParams.fog.color, lightingParams.fog.density]} />` for atmospheric distance separation between the foreground columns, the beam, and the back wall — applies automatically to the standard materials (floor/walls/columns/dust) via three.js's built-in fog support; the custom shaft/floor-pool shaders are intentionally not fogged (they're additive light, not physical surfaces — fogging them would dim the beam itself, which isn't the intent). Fog **color lifted off pure black and density reduced per follow-up review feedback: `#08080a`→`#242428`, density 0.05→0.032** — the previous near-black, denser fog was itself contributing to the back wall reading as lost in blackness. No other Canvas/camera change.
- **Material tone (evaluated, not changed):** the review checklist offered lightening the wall material color/roughness as a third option "if needed." With the ambient and fog adjustments above, the back and side walls are now clearly legible without touching their approved Phase 1A material colors — so this option wasn't applied, to keep the change footprint minimal and avoid touching protected Phase 1A material decisions unless actually necessary.
...
**Current commit (Phase 1B, technically complete):** `d36f88b` — "Phase 1B: raise ambient and lift fog for background wall visibility" (on top of `2da89c8` and `9d7e5b2`, the prior Phase 1B commits)
...
**Approved visual decisions:** None yet — pending human review of this phase.
**Git checkpoint:** `main` branch; initial commit `9d7e5b2`, first refinement `2da89c8`, second refinement `d36f88b`.
